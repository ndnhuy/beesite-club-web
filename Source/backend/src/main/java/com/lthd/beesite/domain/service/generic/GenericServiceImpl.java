package com.lthd.beesite.domain.service.generic;
...
public abstract class GenericServiceImpl<E, D, ID extends Serializable> implements GenericService<E, D, ID>{
	...
	public D findOne(ID id) {
		...
		if (e == null) {
			throw new EntityNotFoundException(entityType.getSimpleName() + "[id = " + id + "] not found",
	...
	public D update(D updatedInfo) {
		...
		try {
			...
			if (!repository.exists(id)) {
				EntityNotFoundException e = new EntityNotFoundException(entityType.getSimpleName() + "[id = " + id + "] not found",
						entityType.getSimpleName() + "[id = " + id + "] not found");
